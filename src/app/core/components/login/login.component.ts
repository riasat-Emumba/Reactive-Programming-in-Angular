...
import { USER_CREDENTIALS } from '../../constants/user.constants';
...
export class LoginComponent implements OnInit {
  ...
  private isLoginSuccessful(userName: string, password: string, isAdmin: boolean): boolean {
    const storedUserName = localStorage.getItem(USER_CREDENTIALS.USERNAME);
  ...
  private handleLogin(userName: string, password: string, isAdmin: boolean): void {
    if (!localStorage.getItem(USER_CREDENTIALS.USERNAME) && !localStorage.getItem(USER_CREDENTIALS.PASSWORD) && !localStorage.getItem(USER_CREDENTIALS.IS_ADMIN)) {
      localStorage.setItem(USER_CREDENTIALS.USERNAME, userName);
  ...
  private validateFormFields(): void {
    if (this.loginFormControls[USER_CREDENTIALS.USERNAME]?.errors?.['required']) {
